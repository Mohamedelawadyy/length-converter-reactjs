...
export default function InputWrapper({
  ...
  const convertToStandardUnit = () => {
    ...
    const currentValue = inputElement.current.value
      ...
      : 0;
    const standardUnitFormulaAfterSubstitution = standardUnitFormula.replace(
      /x/,
      currentValue.toString()
    );
    const standardUnitFormulaSolution = eval(
      removeZerosFromStart(standardUnitFormulaAfterSubstitution)
    );
  ...
  const convertToCurrentUnit = () => {
    ...
    const currentUnitFormulaAfterSubstitution = currentUnitFormula.replace(
      /x/,
      standardUnit.toString()
    );
    const currentValue = eval(
      removeZerosFromStart(currentUnitFormulaAfterSubstitution)
    );
